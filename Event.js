...
function AddTeamMembersToRest() {
    for (i = 0; i < NMaleArray.length; i++) {
        ////不存在 則添加
        if (!checkspanValExist(NMaleArray[i])) {
            PlayerNum++;
            str = '<li class="p' + (String)(PlayerNum) + '"><input id="InputName' + (String)(PlayerNum) + '" type="text"><button class="Name">改名</button><span class="sortHandle">' + NMaleArray[i] + '</span></li>';
            var el = $(str);
    ...
    for (i = 0; i < NFemaleArray.length; i++) {
        if (!checkspanValExist(NFemaleArray[i])) {
            PlayerNum++;
            str = '<li class="p' + (String)(PlayerNum) + '"><input id="InputName' + (String)(PlayerNum) + '" type="text"><button class="Name">改名</button><span class="sortHandle">' + NFemaleArray[i] + '</span></li>';
            var el = $(str);
            $('#Restgrouped').append(el);
            $('#Restgrouped input').css('width', '50px');
            $('#Restgrouped input').hide();
    ...
    for (i = 0; i < NVIPArray.length; i++) {
        if (!checkspanValExist(NVIPArray[i])) {
            PlayerNum++;
            str = '<li class="p' + (String)(PlayerNum) + '"><input id="InputName' + (String)(PlayerNum) + '" type="text"><button class="Name">改名</button><span class="sortHandle">' + NVIPArray[i] + '</span></li>';
            var el = $(str);
            $('#Restgrouped').append(el);
            $('#Restgrouped input').css('width', '50px');
            $('#Restgrouped input').hide();
