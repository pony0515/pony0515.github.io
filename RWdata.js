...
function AddMember(who, name) {
    switch (who) {
        ...
        case 'VIP':
            ...
            checkMArray(who);
            PlayerNum++;
            str = '<li class="p' + (String)(PlayerNum) + '"><input id="InputName' + (String)(PlayerNum) + '" type="text"><button class="Name">改名</button><span style="background-color:hotpink" class="sortHandle">' + result + '</span></li>';
            var el = $(str);
            $('#Restgrouped').append(el);
            $('#Restgrouped button').css('width', ViewPlayersbutton);
            $('#Restgrouped button').css('font-size', ViewPlayersbuttonFontSize);
